...
#define Fout 440
#define Fendswoop 1740
#define Fmaxswoop 2000
#define SwoopSample 5720
#define two32_Fs 97612
#define pi_SwoopSample 0.00054895104
#define song_length 50
volatile unsigned int DDS_increment = Fout * two32_Fs; //42949673 ;
// waveform amplitude
volatile _Accum max_amplitude = 2000;

volatile char swoop = 0;
// Number of times we've entered ISR
volatile unsigned int nsamples = 0;

// waveform amplitude envelope parameters
// rise/fall time envelope 44 kHz samples
volatile unsigned int attack_time = 500, decay_time = 1000, sustain_time = SwoopSample - 1500;
...
// semaphores
static struct pt_sem play, playing;

// song array
volatile int song[song_length];
...
void transitionState(int key, struct pt *pt)
{
    if (key_state == 0)
    {
        if (key == -1)
        ...
        if (key == possible)
        {
            ...
            if (possible == 1)
            {
                ...
                if (recording == 0)
                {
                    swoop = 1;
                    resetMusic();
                    // PT_SEM_SIGNAL(pt, &play);
                    // PT_SEM_WAIT(pt, &playing);
                }
                else
                {
                    song[counter] = 1;
            ...
            if (possible == 2)
            {
                ...
                // call play thread if in play mode
                if (recording == 0)
                {
                    swoop = 0;
                    resetMusic();
...
// Calculates the swoop freq
double swoopFreq(int nsamples)
{
    return 260 * sin((double)nsamples * pi_SwoopSample) + 1740;
}

double chirpFreq(int nsamples)
{
    return 0.000153 * nsamples * nsamples + 2000;
}
...
void __ISR(_TIMER_2_VECTOR, ipl2) Timer2Handler(void)
{
    ...
    // Increment nsamples
    if (nsamples < SwoopSample)
        ++nsamples;
    // Generate desired frequency
    //    int sin_idx = (int) nsamples/(SwoopSample*2);
    //    int freq = 260*sine_table[sin_idx] + 1740
    double freq = 0;
    if (swoop)
    {
        freq = swoopFreq(nsamples);
    }
    else
    {
        freq = chirpFreq(nsamples);
    }
    DDS_increment = (int)freq * two32_Fs;
    // generate  sinewave
    // advance the phase
    ...
    {
        current_amplitude = (note_time <= attack_time) ? current_amplitude + attack_inc : (note_time <= attack_time + sustain_time) ? current_amplitude
...
#define no_button (0x70)
    ...
    while (1)
    {
        ...
        if (i == 22)
            sprintf(buffer, "   %x shift-*", keypad);
        if (i == 23)
            sprintf(buffer, "   %x shift-#", keypad);
        if (i > -1 && i < 12)
            printLine2(10, buffer, ILI9340_GREEN, ILI9340_BLACK);
        else if (i > -1)
            printLine2(10, buffer, ILI9340_RED, ILI9340_BLACK);
        // !!!! NEVER exit while !!!!
    } // END WHILE(1)
    PT_END(pt);
} // keypad thread
...
void main(void)
{
    ...
    while (1)
    {
        ...
        PT_SCHEDULE(protothread_play(&pt_play));
    }
} // main
